...
### Added
- 最终结果浮窗现在支持 Markdown 渲染，同时保留原始文本供复制与恢复使用

### Changed
- 新增结果浮窗的折叠 / 展开切换，折叠后会收成更紧凑的标题栏，以减少遮挡
- 浮窗尺寸测量与定位逻辑现在会区分 partial 与 final 结果的渲染方式，降低恢复与流式更新时的几何错位

### Test
- 补上 Markdown 渲染、原始文本复制、折叠几何还原，以及 partial / final 重绘路径的回归测试

## [1.0.9] - 2026-04-11

### Added
- 手动输入窗口新增可独立编辑的目标语言字段，支持常用语言建议列表，并会记住上一次手动输入使用的目标语言，方便在同一个 API Profile 下反复切换不同输出语言

### Changed
- 手动输入流程改为以对话框内的目标语言作为本次文本请求的最终值，不再强制跟随设置页中的 `Target Language`
- `manual input` 验证范围调整为只要求 API / Model / Text Prompt 等必要字段；即使设置页的 `Target Language` 为空，也可以先打开输入框，再在对话框内补齐本次语言
- `config.json` / `config.example.json` 现在会保存 `manual_input_target_language`，让手动输入工作流可以跨重启沿用最近一次使用的语言

### Fixed
- 修复 Gemini Compatible 请求在严格兼容后端上缺少 `contents[0].role = "user"` 的问题；`Test API`、文本请求与图片请求现在可以正确通过需要完整 Gemini schema 的第三方端点，不再因为 `HTTP 422` 失败

### Test
- 补上 Gemini Compatible `role` 字段、手动输入目标语言覆盖 / 持久化 / 建议列表，以及 `manual_input` 验证范围的回归测试

## [1.0.8] - 2026-04-09

### Changed
- 内建 `翻译 (Translate)` 默认方案的图片 / 文本提示词改为更中性的 OCR / 翻译指令，并加入仅替换触发安全规则片段的 `[REDACTED]` fallback，降低整段翻译被直接拒绝的概率
- 未 Pin 浮窗宽度改为仅在当前运行期间保留；程序关闭后不再把未 Pin 宽度写回 `config.json`，避免旧场景残留的宽度影响下次启动

### Fixed
- 修复冷启动后第一次截图翻译的首个 streamed partial 仍可能从基准宽度起算、再横向跳到校正位置的问题；现在会先根据选区可用空间预估更合理的初始宽度
- 改善首次显示翻译浮窗时的原生窗口预热与 header 测量流程，进一步降低第一次 visible render 的闪动与位置不稳定

### Test
- 补上第一次截图请求宽度 seed、session-only 未 Pin 宽度状态、配置不再持久化未 Pin 宽度，以及首次 visible partial 预热路径的回归测试

## [1.0.7] - 2026-04-08

### Added
- 进阶设置新增可切换的“调试日志”开关，默认只保留普通用户能理解的运行记录；开启后才会额外显示定位诊断、截图规划与 API 底层重试细节

### Changed
- API / runtime 日志进一步分层：用户可感知的 API 重试与流式回退事件现在会保留在普通运行记录中，而较吵的技术性细节改为仅在调试日志模式下显示
- 进阶设置中的说明与开关顺序重新整理，将未 Pin / Pin 几何说明、流式响应、关闭窗口最小化到系统托盘与调试日志的阅读顺序调整得更直观

### Fixed
- 修复左右模式下，截图翻译结果浮窗首次显示时可能低估标题与工具栏宽度，导致位置略微压到选区，并且需要靠字号缩放后才回到正确位置的问题
- 修复流式图片翻译中，首个 partial 浮窗可能先显示在预估位置、再横向跳到校正位置的问题，降低首个 partial 出现时的左右晃动
- 修复流式图片翻译中，浮窗在持续增量更新时容易因反复重定位与全文重绘而闪烁的问题；后续 partial 更新现在会尽量维持位置与宽度稳定，只在安全范围内增加高度，并优先采用 append 方式插入增量文本
- 改善浮窗接近底部边界时的稳定性：如果继续长高会迫使窗口反复向上夹回，现在会优先冻结当前几何以减少抖动

### Test
- 补上浮窗标题宽度测量、首个 partial 校正、持续 partial 几何冻结、增量文本 append 更新，以及诊断日志同时保留 original / corrected rect 的回归测试

## [1.0.6] - 2026-04-07

### Changed
- 截图工作流现在会在每次开始框选前重建新的 `SelectionOverlay`，避免 Windows 透明顶层遮罩短暂重用上一轮画面而出现旧截图闪烁
- 设置页中的动作按钮（如 `Fetch Models`、`Test API`、`Save Settings`、`Check Updates` 等）改为更稳定的焦点管理，降低 busy 状态切换时自动把表单滚到最底的概率
- 选中文本工作流现在会先检查应用内当前焦点控件的文字选区；如果存在可用选字，会直接提交文字请求，不再强制走剪贴板截取

### Fixed
- 修复连续截图时，新的截图遮罩刚出现那一瞬间偶发闪出上一张 frozen snapshot 的问题
- 修复主窗口设置页在点击 `Fetch Models` / `Test API` 等按钮后，因控件禁用与焦点回退而导致画面自动滚到底的问题
- 修复主窗口内部已选中文本时，触发选字翻译快捷键仍可能提示“没有获取到可用的选中文本”的问题

### Test
- 补上每次重建截图遮罩、设置区按钮焦点清理，以及应用内选字直读 / 换行正规化的回归测试
