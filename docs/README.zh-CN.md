...
- 手动输入窗口现在可以为单次请求单独指定目标语言，支持常用语言建议，并会记住上一次手动输入使用的语言
- 第三方 Compatible 后端如果不支持流式，会提示当前状态并自动回退到非流式；流式中断时也会保留部分结果与状态标记
- 设置页采用“连接与模型 → 翻译方式与快捷键 → 界面与进阶”三段式流程，更容易完成首次配置
- 应用内选中文本现在会优先直接读取当前焦点控件的文字选区；如果没有可用的应用内选字，才回退到原本的系统剪贴板抓取路径
- 屏幕框选现在会先冻结桌面快照再裁切选区，降低高 DPI / 多显示器偏移、hover 新 UI 混入与主窗口残影
- 每次截图都会重建新的选择遮罩，降低 Windows 上偶发闪出上一张截图背景的概率
- 设置页中的 `Fetch Models` / `Test API` / `Save Settings` 等动作按钮已补上更稳定的焦点处理，避免 busy 状态切换时把页面自动滚到底
- 左右模式下的结果浮窗现在会更准确地测量标题与工具栏宽度，首次显示时更不容易压到选区
- 流式图片翻译的浮窗更新策略已进一步稳定：首次 partial 会先根据选区可用空间预估宽度，降低冷启动第一次截图时的横向跳位，持续更新时也会尽量避免靠近底部时的抖动
...
- 结果浮窗支持：
  ...
  - 拖拽移动与角落拖拽改尺寸
  - `Ctrl + 鼠标滚轮` 缩放字体
- 已 Pin 的结果浮窗可保留位置与尺寸；未 Pin 状态则会根据当前场景动态展开，手动调整的宽度只在本次运行期间保留，不会跨重启写回配置文件
- 内建 `翻译 (Translate)` 默认方案已更新为更中性的 OCR / 翻译提示词；如果后端安全规则只拦截局部内容，会以 `[REDACTED]` 替换触发片段并尽量完成剩余翻译
- 支持 `浅色 / 深色 / 跟随系统` 三态主题切换
- 支持全局快捷键、系统托盘、单实例唤回与 `--capture` 快速启动
- 进阶设置新增可切换的调试日志；默认只保留普通用户能理解的运行记录，开启后才会额外显示定位诊断、截图规划与 API 底层重试细节
...
- 选中文本快捷键
- 托盘菜单中的截图 / 输入框入口
- 手动输入对话框可以直接覆盖本次请求的目标语言；即使主设置字段暂时留空，也能先打开输入框后再填写
- 对应的全局快捷键
