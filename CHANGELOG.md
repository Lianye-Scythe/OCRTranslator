...
### Added
- 最終結果浮窗現在支援 Markdown 渲染，保留原始文字供複製與還原使用

### Changed
- 新增結果浮窗的折疊 / 展開切換，折疊後會收成更緊湊的標題列，以減少遮擋
- 浮窗尺寸量測與定位邏輯現在會區分 partial 與 final 結果的渲染方式，降低恢復與流式更新時的幾何錯位

### Test
- 補上 Markdown 渲染、原始文字複製、折疊幾何還原，以及 partial / final 重繪路徑的回歸測試
...
### Changed
- 內建 `翻譯 (Translate)` 預設方案的圖片 / 文字提示詞改為更中性的 OCR / 翻譯指令，並加入僅替換觸發安全規則片段的 `[REDACTED]` fallback，降低整段翻譯被直接拒絕的機率
- 未 Pin 浮窗寬度改為僅在當前執行期保留；程式關閉後不再把未 Pin 寬度寫回 `config.json`，避免舊場景殘留的寬度影響下次啟動

### Fixed
- 修正冷啟動後第一次截圖翻譯的首個 streamed partial 仍可能從基準寬度起算、再橫向跳到校正位置的問題；現在會先根據選區可用空間預估更合理的初始寬度
- 改善首次顯示翻譯浮窗時的原生視窗預熱與 header 量測流程，進一步降低第一次 visible render 的閃動與位置不穩定

### Test
- 補上第一次截圖請求寬度 seed、session-only 未 Pin 寬度狀態、配置不再持久化未 Pin 寬度，以及首次 visible partial 預熱路徑的回歸測試

## [1.0.7] - 2026-04-08

### Added
- 進階設定新增可切換的「除錯 Log」開關，預設只保留一般使用者可理解的執行紀錄；開啟後才會額外顯示定位診斷、截圖規劃與 API 底層重試細節

### Changed
- API / runtime Log 進一步分層：使用者可感知的 API 重試與流式回退事件現在會保留在一般執行紀錄中，而較吵的技術性細節則改為僅在除錯 Log 模式下顯示
- 進階設定的說明與開關順序重新整理，將未 Pin / Pin 幾何說明、流式回應、關閉視窗最小化到系統匣與除錯 Log 的閱讀順序調整得更直覺

### Fixed
- 修正左右模式下，截圖翻譯結果浮窗首次顯示時可能低估標題與工具列寬度，導致位置略微壓到選取區，且需靠字級縮放後才回到正確位置的問題
- 修正流式圖片翻譯中，第一個 partial 浮窗可能先顯示在預估位置、再橫向跳到校正位置的問題，降低首個 partial 出現時的左右晃動
- 修正流式圖片翻譯中，浮窗在持續增量更新時容易因反覆重定位與全文重繪而閃爍的問題；後續 partial 更新現在會盡量維持位置與寬度穩定，只在安全範圍內增加高度，且增量文字會優先採用 append 更新
- 改善浮窗接近底部邊界時的穩定性：若持續長高會迫使視窗反覆往上夾回，現在會優先凍結當前幾何以減少抖動

### Test
- 補上浮窗標題寬度量測、首個 partial 校正、持續 partial 幾何凍結、增量文字 append 更新，以及診斷 Log 同時保留 original / corrected rect 的回歸測試

## [1.0.6] - 2026-04-07

### Changed
- 截圖工作流現在會在每次開始框選前重建新的 `SelectionOverlay`，避免 Windows 透明頂層遮罩短暫重用上一輪畫面而出現舊截圖閃爍
- 設定頁中的動作按鈕（如 `Fetch Models`、`Test API`、`Save Settings`、`Check Updates` 等）改為更穩定的焦點管理，降低 busy 狀態切換時自動把表單捲到最底的機率
- 選取文字工作流現在會先檢查應用內目前焦點控件的文字選區；若存在可用選字，會直接送出文字請求，不再強制走剪貼簿擷取

### Fixed
- 修正連續截圖時，新的截圖遮罩剛出現那一瞬間偶發閃出上一張 frozen snapshot 的問題
- 修正主視窗設定頁在點擊 `Fetch Models` / `Test API` 等按鈕後，因控件禁用與焦點回退而導致畫面自動捲到底的問題
- 修正主視窗內部已選取文字時，觸發選字翻譯快捷鍵仍可能回報「沒有取得可用的選取文字」的問題

### Test
- 補上每次重建截圖遮罩、設定區按鈕焦點清理，以及應用內選字直讀 / 換行正規化的回歸測試
